...
//  Copyright (c) 2014 Keeper Security, Inc. All rights reserved.
//
...
@interface KeeperSDK : NSObject

// Keeper specific Extension Interface
+ (KeeperSDK *)sharedExtension;

// Generic Password Extension Interface

// Determines if the Generic Password App Extension is available. Allows you to only show the Generic Password extension buttons to those
// users that can use it. Of course, you could leave the button enabled and educate users about the virtues of strong, unique
// passwords instead :)
// 
// Note that this returns YES if any app that supports the generic `org-appextension-feature-password-management` feature
// is installed.
#ifdef __IPHONE_8_0
- (BOOL)isAppExtensionAvailable NS_EXTENSION_UNAVAILABLE_IOS("Not available as an extension. No installed app supports the org-appextension-feature-password-management:// feature.");
#else
...
// Create a new login within the Generic Password App and allow the user to generate a new password before saving. The provided URLString should be
...
// Change the password for an existing login within the Generic Password App. The provided URLString should be
